...
URL: http://themble.com/mergehealth/
...
// LOAD mergehealth CORE (if you remove this, the theme will break)
require_once( 'library/mergehealth.php' );
...
function mergehealth_ahoy() {

  //Allow editor style.
  add_editor_style( get_stylesheet_directory_uri() . '/library/css/editor-style.css' );

  // let's get language support going, if you need it
  load_theme_textdomain( 'mergehealth', get_template_directory() . '/library/translation' );

  // USE THIS TEMPLATE TO CREATE CUSTOM POST TYPES EASILY
  require_once( 'library/custom-post-type.php' );

  // launching operation cleanup
  add_action( 'init', 'mergehealth_head_cleanup' );
  // A better title
  add_filter( 'wp_title', 'rw_title', 10, 3 );
  // remove WP version from RSS
  add_filter( 'the_generator', 'mergehealth_rss_version' );
  // remove pesky injected css for recent comments widget
  add_filter( 'wp_head', 'mergehealth_remove_wp_widget_recent_comments_style', 1 );
  // clean up comment styles in the head
  add_action( 'wp_head', 'mergehealth_remove_recent_comments_style', 1 );
  // clean up gallery output in wp
  add_filter( 'gallery_style', 'mergehealth_gallery_style' );

  // enqueue base scripts and styles
  add_action( 'wp_enqueue_scripts', 'mergehealth_scripts_and_styles', 999 );
  // ie conditional wrapper

  // launching this stuff after theme setup
  mergehealth_theme_support();

  // adding sidebars to Wordpress (these are created in functions.php)
  add_action( 'widgets_init', 'mergehealth_register_sidebars' );

  // cleaning up random code around images
  add_filter( 'the_content', 'mergehealth_filter_ptags_on_images' );
  // cleaning up excerpt
  add_filter( 'excerpt_more', 'mergehealth_excerpt_more' );

} /* end mergehealth ahoy */
...
// Thumbnail sizes
add_image_size( 'mergehealth-thumb-600', 600, 150, true );
add_image_size( 'mergehealth-thumb-300', 300, 100, true );
...
<?php the_post_thumbnail( 'mergehealth-thumb-300' ); ?>
for the 600 x 150 image:
<?php the_post_thumbnail( 'mergehealth-thumb-600' ); ?>

You can change the names and dimensions to whatever
you like. Enjoy!
*/

add_filter( 'image_size_names_choose', 'mergehealth_custom_image_sizes' );

function mergehealth_custom_image_sizes( $sizes ) {
    return array_merge( $sizes, array(
        'mergehealth-thumb-600' => __('600px by 150px'),
        'mergehealth-thumb-300' => __('300px by 100px'),
    ) );
}
...
function mergehealth_theme_customizer($wp_customize) {
...
add_action( 'customize_register', 'mergehealth_theme_customizer' );

/************* ACTIVE SIDEBARS ********************/

// Sidebars & Widgetizes Areas
function mergehealth_register_sidebars() {
	register_sidebar(array(
		'id' => 'sidebar1',
		'name' => __( 'Sidebar 1', 'mergehealth' ),
		'description' => __( 'The first (primary) sidebar.', 'mergehealth' ),
	...
	register_sidebar(array(
		'id' => 'sidebar2',
		'name' => __( 'Sidebar 2', 'mergehealth' ),
		'description' => __( 'The second (secondary) sidebar.', 'mergehealth' ),
		'before_widget' => '<div id="%1$s" class="widget %2$s">',
		'after_widget' => '</div>',
		'before_title' => '<h4 class="widgettitle">',
		'after_title' => '</h4>',
...
// Comment Layout
function mergehealth_comments( $comment, $args, $depth ) {
    ...
    <article  class="cf">
      <header class="comment-author vcard">
        ...
        */
        ...
        <?php printf(__( '<cite class="fn">%1$s</cite> %2$s', 'mergehealth' ), get_comment_author_link(), edit_comment_link(__( '(Edit)', 'mergehealth' ),'  ','') ) ?>
        <time datetime="<?php echo comment_time('Y-m-j'); ?>"><a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ?>"><?php comment_time(__( 'F jS, Y', 'mergehealth' )); ?> </a></time>

      </header>
      <?php if ($comment->comment_approved == '0') : ?>
        <div class="alert alert-info">
          <p><?php _e( 'Your comment is awaiting moderation.', 'mergehealth' ) ?></p>
        </div>
      <?php endif; ?>
...
function mergehealth_fonts() {
  ...
  wp_enqueue_style('googleLobsterFonts', 'https://fonts.googleapis.com/css?family=Lobster+Two:400,700');
}
